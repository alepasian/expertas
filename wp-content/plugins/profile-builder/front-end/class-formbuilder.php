				...
				if( ( 'POST' == $_SERVER['REQUEST_METHOD'] ) && ( $_POST['action'] == $this->args['form_type'] ) ) {
                    ...
                    if( $this->args['form_type'] == 'register' ) {
                        ...
                        switch( $account_management_settings ) {
                            case 'ec-no_aa-no':
                                $wppb_register_success_message = apply_filters( 'wppb_register_success_message', sprintf( __( "Tu cuenta %1s ha sido creada con exito ingresa aquí para editar tu perfil", 'profile-builder' ), $account_name ), $account_name );
                                break;
                            case 'ec-yes_aa-no':
                                $wppb_register_success_message = apply_filters( 'wppb_register_success_message', sprintf( __( "Tu cuenta %1s ha sido creada, chequea tu casilla de mail para activar la cuenta, recuerda de revisar tu correo no deseado o spam ", 'profile-builder' ), $account_name ), $account_name );
                                break;
                            case 'ec-no_aa-yes':
								if( current_user_can( 'delete_users' ) ) {
									$wppb_register_success_message = apply_filters( 'wppb_register_success_message', sprintf( __( "La cuenta %1s ha sido creada", 'profile-builder' ), $account_name ), $account_name );
								} else {
									$wppb_register_success_message = apply_filters( 'wppb_register_success_message', sprintf( __( "Tu cuenta %1s ha sido creada, chequea tu casilla de mail para activar la cuenta, recuerda de revisar tu correo no deseado o spam ", 'profile-builder' ), $account_name ), $account_name );
								}
								break;
						...
						echo $form_message_tpl_start  . apply_filters( 'wppb_edit_profile_success_message', __( 'Tu perfil esta actualizado!', 'profile-builder' ) ) . $form_message_tpl_end . $redirect;
